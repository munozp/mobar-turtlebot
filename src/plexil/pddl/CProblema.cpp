...
CProblema::CProblema(char* inicio, char* objetivos)
{
    ...
    {//Valores por defecto
        strcpy(inits,INIT);
        strcpy(metas,GOAL);
        strcpy(configuracion,CONFIG);//Añadido Diego
        strcpy(objs,OBJS);//Añadido Diego
    }
    else
    {
        strcpy(inits,inicio);
...
void CProblema::init(char* rutaprob, char* rutaaux, char* inicio, char* objetivos)
{
    ...
    if(rutaprob[strlen(rutaprob)-1] != '~')
    {
        char newrutaprob[512];
        strcpy(newrutaprob, rutaprob);
        strcat(newrutaprob, (char*)"~");
        ifstream source(rutaprob, ios::binary);
        ofstream dest(newrutaprob, ios::binary);
        dest << source.rdbuf();
        source.close();
        dest.close();
        strcpy(rutaprob, newrutaprob);
    }
...
bool CProblema::eliminar_goals()
{
    ...
    int chleidos;
    chleidos=copiar_hasta(metas,&entrada,&salida,true);
    //Previene incluir informacion al final del archivo
    if(chleidos==0)
        do{
            entrada.getline(buffer,TAM_LINEA);
        	chleidos=strlen(buffer);
        }while(chleidos>0 && strncmp(buffer,(char*)")",1));
    salida.write(buffer, strlen(buffer));
    salida.write((char*)"\n",1);
    ...
    return (actualizar_archivos() == 0);
}

bool CProblema::eliminar_inits()
{
    fstream entrada(rutaproblema,ios::in);
    fstream salida(rutaauxiliar,ios::out);
    ...
    char buffer[TAM_LINEA];
    int chleidos;
    chleidos=copiar_hasta(inits,&entrada,&salida,true);
    //Previene incluir informacion al final del archivo
    if(chleidos==0)
        do{
            entrada.getline(buffer,TAM_LINEA);
        	chleidos=strlen(buffer);
        }while(chleidos>0 && buffer[0]!=';');
    salida.write(buffer, strlen(buffer));
    salida.write((char*)"\n",1);
    //Copiar el resto hasta el final
    copiar_hasta(NULL,&entrada,&salida,true);
...
bool CProblema::eliminar_objs()
{
    ...
    char buffer[TAM_LINEA];
    int chleidos;
    chleidos=copiar_hasta(objs,&entrada,&salida,true);
    //Previene incluir informacion al final del archivo
    if(chleidos==0)
        do{
            entrada.getline(buffer,TAM_LINEA);
...
int CProblema::insertar_pred(char* pred, char* objs, bool meta)
{
    ...
    if(!meta)
        ...
        encontrado=copiar_hasta(configuracion,&entrada,&salida,true);//Modificada por Diego. La original es la línea anterior.
    else
