...
/*    int PIDplanificador=fork();
    ...
    if(PIDplanificador)
    {
        ...
        waitpid(PIDplanificador,&estadosal,NULL);
    }
    else //Hijo, ejecucion del planificador
    {
        char cmdp[2048];
        sprintf(cmdp,"%s %s %s %s %s %s %s",planificador[0],planificador[1],planificador[2],planificador[3],planificador[4],planificador[5],planificador[6]);
        system(cmdp);
    }
...
*/
    char orden[2048];
    sprintf(orden,"%s %s %s %s %s %s %s\n",planificador[0],planificador[1],planificador[2],planificador[3],planificador[4],planificador[5],planificador[6]);
    estadosal = system(orden);
...
char * Planner::obtenerRutaMapaCostes()
{
    //printf("Planner\n%s\n",planificador[5]);
    return planificador[5];
}
